...
public class RodapeController {
	...
	public String newsletter(Model model, HttpServletRequest req, Newsletter nw)
	{
		...
		if(inseriu)
		{
			...
			String mensagem = String.format("O e-mail: %s acaba de se cadastrar para receber novas histórias", nw.getEmail());
			Email.enviaEmail(nw.getEmail(), "Cadastro de newsletter", mensagem, GlobalEnum.ANEXO_VAZIO.getValor());
		}
		else
			model.addAttribute("mensagem", String.format("Seu e-mail não pode ser cadastrado, envie um e-mail para %s", utils.getProp("emailadministrador")));
